...
void SOP_member_statement(SOP_class *SOP)
{
   ...
   if (LEX_type(SOP->LEX,LEX_CUR) != TTYP_STRLIT)
      ...
      if (!SOP_export_symbol(SOP,(ULONG) "PARENT",SOP->parent,'N',1))
...
UWORD *SOP_add_break(SOP_class *SOP, UWORD label)
{
   ...
   SOP->bsp = add_ptr(SOP->bsp,(ULONG) sizeof(UWORD));

   if (ptr_dif(SOP->bsp,&SOP->b_stack[MAX_NEST]) < 0L)
      *SOP->bsp = label;
   else
...
UWORD *SOP_add_continue(SOP_class *SOP, UWORD label)
{
   ...
   SOP->csp = add_ptr(SOP->csp,(ULONG) sizeof(UWORD));

   if (ptr_dif(SOP->csp,&SOP->c_stack[MAX_NEST]) < 0L)
      *SOP->csp = label;
   else
      report(E_ERROR,LEX_line(SOP->LEX,LEX_CUR),MSG_TML);
...
void SOP_continue_statement(SOP_class *SOP)
{
   ...
   lvl = ptr_dif(SOP->csp,SOP->c_stack);
...
void SOP_break_statement(SOP_class *SOP)
{
   ...
   lvl = ptr_dif(SOP->bsp,SOP->b_stack);
...
void SOP_switch_statement(SOP_class *SOP)
{
   ...
   while ((entry = DI_fetch(DI)) != NULL)
      if (!strcmp(entry->tag,KW_DEFAULT))
         def = (UWORD) ascnum(entry->def,10);
      else
         {
         SOP_emit_long(SOP,ascnum(entry->tag,10));
         SOP_emit_word(SOP,(UWORD) ascnum(entry->def,10));
         ++case_cnt;
         }
   DI_destroy(DI);
...
void SOP_message_statement(SOP_class *SOP)
{
   ...
   DICT_wipe(SOP->APUS);
   DICT_wipe(SOP->AUTO);
   DICT_enter(SOP->AUTO,KW_THIS,0)->def = "2,0,2";
   SOP->auto_index = 2;
...
ULONG SOP_write_dict(SOP_class *SOP, BYTE *suffix, DICT_class *DICT)
{
   ...
   name = mem_alloc(strlen(SOP->name)+strlen(suffix)+1L);
...
WORD SOP_new_source_attrib(SOP_class *SOP)
{
   ...
   if (cur == NULL)
      ...
      if (ascnum(cur->def,10) != SOP->attrib)
         {
...
SOP_class *SOP_construct(RS_class *RS, ULONG def_attribs)
{
   ...
   SOP = mem_alloc(sizeof(SOP_class));
...
WORD SOP_test(SOP_class *SOP)
{
   ...
   ftime = TS_latest_file_time(SOP->RS->TS,cur->def);
...
void SOP_compile(SOP_class *SOP)
{
   ...
   while (SOP_next_definition(SOP))
      ...
      SOP->bsp = add_ptr(SOP->b_stack,-(ULONG) sizeof(UWORD));
      SOP->csp = add_ptr(SOP->c_stack,-(ULONG) sizeof(UWORD));
