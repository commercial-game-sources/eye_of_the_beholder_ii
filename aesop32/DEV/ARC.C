...
void main(int argc, BYTE *argv[])
{
   ...
   for (i=1;i<argc;i++)
      if ((argv[i][0] != '/') && (argv[i][0] != '-'))
         ...
         if (!stricmp(&argv[i][1],"n"))
            flags |= RS_REBUILD;

         else if (!stricmp(&argv[i][1],"w0"))
            set_errorlevel(E_NOTICE);

         else if (!stricmp(&argv[i][1],"w1"))
            set_errorlevel(E_WARN);

         else if (!stricmp(&argv[i][1],"w2"))
            set_errorlevel(E_ERROR);

         else if (!stricmp(&argv[i][1],"x"))
            flags |= RS_NOUNIQUECHECK;

         else if (!stricmp(&argv[i][1],"v"))
            set_verbosity(1);

         else if (!stricmp(&argv[i][1],"sv"))
            flags |= RS_SHOWVERBOSE;

         else if (!stricmp(&argv[i][1],"sb"))
            flags |= RS_SHOWBRIEF;

         else if (!stricmp(&argv[i][1],"kp"))
